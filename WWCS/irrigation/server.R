server <- function(input, output, session) {
        ...
        fluidRow(
          ...
          splitLayout(
            ...
            selectInput(
              "type",
              "Irrigation Type",
              choices = c("channel", "traditional", "pump", "counter"),
              selected = "counter"
            )
          ),
          br(),
          ...
          splitLayout(
            ...
            checkboxInput("irrigation",
                          width = "200px",
                          label = HTML("<b>Activate Service?</b>"))
          ),
          br(),
          br(),
  ...
  observeEvent(input$submit_edit, priority = 20, {
    ...
    pool::dbExecute(
      pool,
      sprintf(
        'UPDATE Sites SET fieldproperties = JSON_SET(fieldproperties, "$.StartDate", ?, "$.WP", ?, "$.FC", ?, "$.Crop", ?, "$.area", ?, "$.type", ?, "$.humanID", ?) WHERE siteID = ("%s")',
        station_selection
      ),
      params = list(
        as.Date(input$sd),

...
repeat {
  ...
  if (length(id) > 0) {
    ...
    for (i in 1:length(dat$PID)) {
      ...
      system(paste("sudo lsof -p", PID, "| grep /opt > lsof.log"))
      netstat <- readLines("netstat.log")
      lsof <- readLines("lsof.log")
      dat$usr[i] <-
        length(grep("ESTABLISHED", netstat) & grep("tcp", netstat))
      dat$app[i] <-
        regmatches(lsof, regexec("opt/(.*)", lsof))[[1]][2]
    }
    if (!is.null(Dat)) {
